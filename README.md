...
南网接口客户端使用 Home Assistant 管理的异步 HTTP 会话。添加集成时会先选择网络协议，
默认“自动选择 IPv4/IPv6”；之后可随时在集成的“参数设置”中调整。仅 IPv4 只解析、连接
IPv4；仅 IPv6 只解析、连接 IPv6 且不会回退；自动模式在 IPv6 可解析但不可达的环境中
可能产生连接延迟。
